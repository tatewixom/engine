...
namespace Nuke
{
  ...
  static DataPointer getAttributeData(const tinygltf::Model& model, const tinygltf::Accessor& accessor)
  {
    ...
    default:
      std::cerr << "ERROR::MODEL.CPP::GETATTRIBUTEDATA()::"
        "UNSUPPORTED_COMPONENT_TYPE\n";
      throw std::runtime_error("UNSUPPORTED_COMPONENT_TYPE");
    }
  }

  template <typename T> const T* GetAttributeData(const tinygltf::Model& model, const tinygltf::Accessor& accessor)
  {
    const tinygltf::BufferView& bufferView = model.bufferViews[accessor.bufferView];
    const tinygltf::Buffer& buffer = model.buffers[bufferView.buffer];
  ...
  template <typename T> std::vector<T> load_indices(const tinygltf::Model& model, const tinygltf::Primitive& primitive)
  {
  ...
  void Model::loadMeshes(tinygltf::Model& model)
  {
    ...
    for (std::size_t i{ 0 }; i < model.meshes.size(); ++i)
    {
      ...
      for (auto& primitive : mesh.primitives)
      {
        ...
        /* extracting position, texture, and normal
         * accessors */
        vertices.resize(positionsCount);

        // extracting positions. MUST HAVE POSITIONS OR
        ...
        /*
          1. Omitted checking for null pointers on every
          for-iteration.
          2. Omitted manually setting values for Vertex
          members if pointer was null. That is: if
          (normals)
            ...
          else
            vertices[i].position = glm::vec3(0.f);

          Also reconstructed the for-loops to seperately
          do normals, texcoords, and poisitons
          respectively.

          THIS ASSUMES THAT THE VECTOR VALUE INITIALIZES
          VERTEX's DATA MEMBERS
        */
        ...
        {
          /*
      need to fix adding making three
      different types of vectors to send
      indices. the current code only
      supports unsigned shorts, so model
      like the alien load incorrectly.
    */
  ...
  void Model::initialize_t(std::string_view gltf_file_path, glm::vec3&& position)
  {
    ...
    /* import */
    std::filesystem::path abs_gltf_file_path{ File::get_executable_dir() / gltf_file_path };
    tinygltf::Model model{ make_gltf_model(abs_gltf_file_path.string()) };
