...
namespace Nuke
{
  ...
  public:
    template <typename T>
    void initialize(const std::vector<Vertex>& vertices, const std::vector<T>& indices, std::vector<Attribute>&& attributes, std::vector<Matter::Material>&& textures)
    {
  ...
  protected:
    ...
    Element element_{};
    std::vector<Matter::Material> textures_{};
  };
}